...
test('measure pixel generation and PNG encoding separately', async ({ page }, info) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    for (const size of [256, 512]) {
      ...
      for (let i = 0; i < 35; i++) {
        ...
        if (i >= 5) { pixels.push(rendered - start); encoding.push(performance.now() - encodeStart); }
      }
      measurements.push({ size, samples: 30, render: summarize(pixels), encode: summarize(encoding), pngBytes: bytes });
    }
  ...
  expect(result.measurements.every(m => m.pngBytes > 0)).toBe(true);
